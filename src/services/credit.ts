...
export async function getUserCredits(user_uuid: string): Promise<UserCredits> {
  ...
  try {
    ...
    const creditBalance = await getUserCreditBalance(user_uuid);
    if (creditBalance) {
      user_credits.left_credits = creditBalance.balance || 0;
      user_credits.total_credits = creditBalance.total_earned || 0;
      user_credits.used_credits = creditBalance.total_spent || 0;
    } else {
